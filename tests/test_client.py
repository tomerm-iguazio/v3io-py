...
class Test(unittest.TestCase):
    def setUp(self):
        self._client = v3io.dataplane.Client(
            transport_kind="httpclient", logger_verbosity="DEBUG", transport_verbosity="DEBUG"
        )
    ...
    def _delete_dir(self, path):
        response = self._client.container.list(
            container=self._container, path=path, raise_for_status=v3io.dataplane.RaiseForStatus.never
        )
        ...
        for common_prefixes in response.output.common_prefixes:
            self._client.object.delete(container=self._container, path=common_prefixes.prefix)
...
class TestContainer(Test):
    ...
    def test_get_container_contents_invalid_path(self):
        response = self._client.container.list(
            container=self._container, path="/no-such-path", raise_for_status=v3io.dataplane.RaiseForStatus.never
        )
...
class TestStream(Test):
    ...
    def test_delete_stream_with_cg(self):
        ...
        # create a stream
        self._client.stream.create(container=self._container, stream_path=self._path, shard_count=num_shards)

        # write data to all shards so there are files
        for shard_id in range(num_shards):
            self._client.stream.put_records(
                container=self._container,
                stream_path=self._path,
                records=[{"shard_id": shard_id, "data": "data for shard {}".format(shard_id)}],
            )
    ...
    def test_stream(self):
        ...
        response = self._client.stream.put_records(container=self._container, stream_path=self._path, records=records)
        ...
        response = self._client.stream.seek(
            container=self._container, stream_path=self._path, shard_id=1, seek_type="EARLIEST"
        )

        self.assertNotEqual("", response.output.location)

        response = self._client.stream.get_records(
            container=self._container, stream_path=self._path, shard_id=1, location=response.output.location
        )
        ...
        self._client.stream.update(container=self._container, stream_path=self._path, shard_count=16)
    ...
    def _stream_exists(self):
        response = self._client.stream.describe(
            container=self._container, stream_path=self._path, raise_for_status=v3io.dataplane.RaiseForStatus.never
        )
...
class TestObject(Test):
    ...
    def test_append(self):
        ...
        for content in contents:
            self._client.object.put(container=self._container, path=self._object_path, body=content, append=True)
    ...
    def test_get_offset(self):
        self._client.object.put(container=self._container, path=self._object_path, body="1234567890")

        # get the contents without limit
        response = self._client.object.get(container=self._container, path=self._object_path, offset=4)

        self.assertEqual(response.body.decode("utf-8"), "567890")

        # get the contents with limit
        response = self._client.object.get(container=self._container, path=self._object_path, offset=4, num_bytes=3)
    ...
    def test_batch(self):
        ...
        for object_idx in range(num_objects):
            self._client.batch.object.put(self._container, _object_path(object_idx), body=_object_contents(object_idx))
...
class TestSchema(Test):
    ...
    def test_create_schema(self):
        # write schema
        self._client.kv.create_schema(
            container=self._container,
            table_path=self._schema_dir,
            key="key_field",
            fields=[
                {"name": "key_field", "type": "string", "nullable": False},
                {"name": "data_field_0", "type": "long", "nullable": True},
        ...
        # verify the scehma
        response = self._client.object.get(
            container=self._container, path=self._schema_path, raise_for_status=v3io.dataplane.RaiseForStatus.never
        )
...
class TestKv(Test):
    ...
    def test_kv_array(self):
        ...
        self._client.kv.put(container=self._container, table_path=self._path, key=item_key, attributes=item)

        for attribute_name in item.keys():
            self._client.kv.update(
                container=self._container,
        ...
        # get the item
        response = self._client.kv.get(container=self._container, table_path=self._path, key=item_key)
    ...
    def test_kv_values(self):
        ...
        item = {
            item_key: {
                ...
                "feature_unicode": "mustache",
                "numeric_str": "1",
                "unicode": "\xd7\xa9\xd7\x9c\xd7\x95\xd7\x9d",
                "male": True,
                "happy": False,
        ...
        self._client.kv.put(container=self._container, table_path=self._path, key=item_key, attributes=item[item_key])
    ...
    def test_kv(self):
        ...
        self._client.kv.update(
            ...
            table_path=self._path,
            key="louise",
            attributes={"height": 130, "quip": "i can smell fear on you"},
        )

        response = self._client.kv.get(
            container=self._container,
            table_path=self._path,
            key="louise",
        ...
        response = self._client.kv.scan(
            container=self._container, table_path=self._path, filter_expression="feature == 'singing'"
        )
        ...
        for segment in range(total_segments):
            received_items = self._client.kv.new_cursor(
                container=self._container, table_path=self._path, segment=segment, total_segments=total_segments
            ).all()
        ...
        self._client.kv.update(
            container=self._container, table_path=self._path, key="louise", expression="age = age + 1"
        )

        response = self._client.kv.get(
            container=self._container, table_path=self._path, key="louise", attribute_names=["age"]
        )
    ...
    def test_limit(self):
        ...
        # limit using all()
        received_items = self._client.kv.new_cursor(container=self._container, table_path=self._path, limit=30).all()
    ...
    def test_batch(self):
        ...
        for item_key in items.keys():
            self._client.batch.kv.get(
                container=self._container, table_path=self._path, key=item_key, attribute_names=["__size", "age"]
            )
    ...
    def _delete_items(self, path, items):
        ...
        # delete dir
        self._client.object.delete(container=self._container, path=path)

    def _verify_items(self, path, items):
        items_cursor = self._client.kv.new_cursor(container=self._container, table_path=path, attribute_names=["*"])
...
class TestRaiseForStatus(Test):
    ...
    def test_always_raise_no_error(self):
        ...
        self._client.container.list(
            self._container, "/", raise_for_status=v3io.dataplane.transport.RaiseForStatus.always
        )
    ...
    def test_never_raise(self):
        self._client.object.get(
            container=self._container, path="/non-existing", raise_for_status=v3io.dataplane.RaiseForStatus.never
        )
...
class TestCustomTransport(unittest.TestCase):
    def test_verifier_transport(self):
        ...
        # do an item get
        response = client.kv.get(container=container_name, table_path="some/table/path", key="some_item_key")
